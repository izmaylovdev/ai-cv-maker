...
@Component({
  ...
  template: `
    <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-xl p-6 w-72 flex flex-col gap-4">
      ...
      <div cdkDropList (cdkDropListDropped)="drop($event)" class="flex flex-col gap-2">
        @for (key of draft; track key) {
          <div cdkDrag cdkDragLockAxis="y"
            class="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-gray-200 dark:border-zinc-600 bg-gray-50 dark:bg-zinc-700">
            ...
            <span cdkDragHandle class="material-icons text-[20px] text-gray-400 dark:text-zinc-500 cursor-grab active:cursor-grabbing select-none">drag_indicator</span>
            <span class="text-sm font-medium text-gray-700 dark:text-zinc-200">{{ label(key) }}</span>
          </div>
        }
      </div>
      <div class="flex gap-2">
        ...
        <button type="button" (click)="ref.close()"
          class="flex-1 border border-gray-300 dark:border-zinc-600 bg-white dark:bg-transparent text-gray-600 dark:text-zinc-400 hover:bg-gray-50 dark:hover:bg-zinc-700 font-medium rounded-lg px-4 py-2 text-sm transition-colors cursor-pointer">
          Cancel
        </button>
      </div>
    </div>
